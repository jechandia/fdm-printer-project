...
- **Do not issue `HEAD /api/v1/files/...` against the XL.** Its firmware returns a malformed HTTP response (Node throws `Parse Error: Expected HTTP/`) and the request leaves the targeted file locked — subsequent `DELETE` then returns `409` indefinitely (even with `Force`, and via the legacy endpoint), clearable only from the front panel / reboot. The adapter does **not** use `HEAD`; keep it that way for PrusaLink.
